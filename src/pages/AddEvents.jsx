...
const AddEvent = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormMessage('Event added successfully!');
  ...
  if (loading && !formMessage && !error) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <p className="text-xl text-gray-700">Submitting event...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#f5f8ff] to-white py-16 px-6">
      ...
      >
        ...
        <h1 className="text-5xl font-extrabold text-[#0A174E] mb-4">Add New Event</h1>
        <p className="text-xl text-gray-700 max-w-2xl mx-auto">
          Fill in the details to add a new event to the university calendar.
        </p>
      </motion.div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {loading ? 'Adding Event...' : (
              <>
                <PlusCircle className="h-6 w-6 mr-2" /> Add Event
              </>
            )}
          </motion.button>
        </form>
